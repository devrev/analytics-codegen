...
Initially, the widget configuration should look like this:
...
Table
Choose when there are a lot of columns to display.
...
Line, Bar, Column, PackedBubble, Donut, Pie and Heatmap are not supported for a lot of columns. Use Table instead.
...
`is_stacked` is optional and it is used when you have done group by twice
...
Ensure final JSON configuration format follows this sample template.
Remove any added fields that are not mentioned within this guide.
